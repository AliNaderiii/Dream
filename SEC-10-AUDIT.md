...
- **Fix commit (head after this file was written):** `fix(bridge): make the SEC-10 suite host-independent and clippy-clean` — touches only `tests/test_gateway_server.py`, `apps/desktop/src-tauri/src/bridge/process.rs` and this file; no production behaviour changes. Its SHA and the CI / Desktop CI run URLs for it are recorded in the PR body and in the final report.
