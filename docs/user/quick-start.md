...
### 2. Tauri installer (from the `v0.4.6` release onward)

Download `Dream_0.4.6_x64-setup.exe` from the GitHub Release, install it, and
run Dream. On Windows the kernel is inside the installer — no separate
`pip install`, no local Python required:

1. Download **`Dream_0.4.6_x64-setup.exe`** from the
   [GitHub Release](https://github.com/AliNaderiii/Dream/releases) for `v0.4.6`.
