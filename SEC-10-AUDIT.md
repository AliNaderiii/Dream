...
| Python tests | `tests/test_bridge_methods.py`, `tests/security/test_sec_integration.py`, `tests/test_gateway_server.py` | call-site updates only (`run(...)` / `asyncio.run(...)` around the now-async handlers) |
| Rust | `apps/desktop/src-tauri/src/bridge/reader.rs` (new) | bounded frame reader (cancellation-safe discard) + 11 tests |
| Rust | `apps/desktop/src-tauri/src/bridge/process.rs` | supervisor loop, heartbeat, reader loop, writer signal, `SupervisorTiming`, `SupervisorControl`; 10 new tests |
...
| R2 | H | `run_supervisor` **returned** after 3 failures; `Bridge::restart` only dropped the writer. With no supervisor task alive, "Reconnect" did nothing forever. | Supervisor never returns: parks in `Disconnected` on `SupervisorControl::wait_for_restart()`. `Bridge::restart` calls `request_restart()` (level flag + `Notify`, so a request during teardown/backoff is not lost). Manual restart ⇒ fresh budget, no backoff. 30 s healthy uptime resets the budget. | `unix_heartbeat_kills_hung_…` (Disconnected → manual restart → Ready → round-trip), `restart_request_is_remembered_until_consumed`, `wait_for_restart_returns_immediately_for_an_earlier_request` |
...
4. Manual restart requests are level-triggered and consumed once; a request that arrives before the supervisor waits is not lost; a stale wake-up cannot fake a request (flag checked in the loop).
...
**Timings** (production defaults unchanged, now in `SupervisorTiming::default()`, asserted by `default_timing_matches_documented_constants`): heartbeat 5 s / 15 s, backoff 2/5/10 s, new `STABLE_AFTER` 30 s. Tests inject millisecond timings through `SidecarConfig.timing`.
...
5. **`STABLE_AFTER` (30 s) is new policy**, not a wire change: an instance that crashes every 31 s will restart indefinitely (with 2 s backoff each time) rather than giving up after 3. That is the intended trade-off (the previous behaviour permanently disconnected the app after 3 crashes in a day) and is visible in the state indicator.
...
_This section is filled in once the branch is pushed and the PR is open; every claim below is tied to the recorded head SHA._

- **PR:** _pending_
- **Final head SHA:** _pending_
- **CI (`ci.yml`, Python matrix):** _pending_
- **Desktop CI (`desktop-ci.yml`, Frontend + Rust ubuntu/windows/macos):** _pending_
- **Merged:** no. **v0.4.7 created:** no.
