...
import { h, fmtBytes, fmtTime } from '../lib/dom.js';
...
export function voiceView(root, ctx) {
  ...
  function engineChip() {
    if (!transcript) {
      return h('span', { class: 'chip voice-engine-chip' }, h('span', { class: 'dot' }), 'موتور رونویسی: نامشخص');
    }
    if (transcript.simulated) {
      return h('span', { class: 'chip warn voice-engine-chip' }, h('span', { class: 'dot' }), `موتور داخلی (${transcript.engine || 'built-in'})`);
    }
    return h('span', { class: 'chip ok voice-engine-chip' }, h('span', { class: 'dot' }), transcript.engine || 'faster-whisper');
  }
      ...
      children.push(
        h('div', { class: 'voice-actions' },
          h('button', {
            ...
            disabled: !!busy,
            onclick: transcribe,
          }, h('span', { html: ic('wave') }), busy === 'stt' ? 'در حال رونویسی…' : 'رونویسی'),
          h('button', {
            class: 'btn',
        ...
        children.push(
          h('div', { class: 'result-panel card' },
            ...
            h('div', { class: 'result-meta' },
              h('span', { class: 'chip' }, h('span', { class: 'dot' }), `مدت: ${transcript.duration ?? '—'} ثانیه`),
              h('span', { class: 'chip' }, h('span', { class: 'dot' }), `زبان: ${transcript.language || '—' }`),
              transcript.simulated
                ? h('span', { class: 'chip warn' }, h('span', { class: 'dot' }), 'موتور داخلی — برای faster-whisper نصب کامل را نصب کنید')
                : null)),
        );
      }
      if (pdfResult?.success) {
        children.push(
          h('div', { class: 'notice ok', html: ic('check') },
    ...
    try {
      ...
      ctx.openEvidence({
        ...
        steps: [
          ...
          { name: 'stt.transcribe', detail: `${transcript.engine || '—'} · ${transcript.duration ?? '—'}s`, meta: 'هسته پایتون' },
  ...
  const drop = h(
    'div',
    {
      ...
      ondrop: (e) => {
        ...
        renderStage({ error: 'برای مسیر واقعی، از دکمه «انتخاب فایل» استفاده کنید — کشیدن‌ورها کردن فقط داخل اپ دسکتاپ پشتیبانی می‌شود.' });
      },
    },
    stage,
  );
  ...
  root.append(
    h('div', { class: 'voice-view' },
      h('div', { class: 'voice-toolbar' },
        h('button', { class: 'btn', onclick: choose }, h('span', { html: ic('upload') }), 'انتخاب فایل')),
      drop),
  );
